  ...
  <Card shadow>
    <div>
      ...
      <div class="message-page-con message-list-con">
        ...
        >
          <MenuItem v-for="(item,index) in messageUnreadList" :name="index" :key="index">
            <div>
              <p class="msg-title">{{ item.message.slice(0, 10) }}</p>
              ...
              <Button
                ...
                @click.native.stop="removeMsg(index)"></Button>
            </div>
          </MenuItem>
        </Menu>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleView (index) {
      ...
      this.messageContent = this.messageUnreadList[index].message
